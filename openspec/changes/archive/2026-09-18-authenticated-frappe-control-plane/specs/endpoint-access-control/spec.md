...
Credential-gated legacy inference endpoints, a minimal public health exception, explicit dual-opt-in development and no direct Desk access. Endpoint retention does not authorize end users or complete G2/M2.
...
Existing direct routes SHALL remain present. Legacy direct `/orchestrate` without gateway-envelope fields MAY retain caller-mode behavior behind the credential or explicit development exemption; it SHALL NOT be labeled authenticated Frappe-user context. Gateway envelopes SHALL instead obey gateway validation and backend chat-only scope, never downgrade to legacy behavior.

#### Scenario: Route inventory comparison
- **WHEN** inference routes are compared before and after implementation
- **THEN** every previously existing route remains present behind its specified access rules

#### Scenario: Legacy and gateway separation
- **WHEN** a valid direct legacy request has no gateway envelope
- **THEN** legacy behavior remains available under service access rules without authenticated user attribution, while a partial or malformed gateway envelope is refused
...
Desk SHALL send chat through authenticated Frappe only. It SHALL NOT call legacy/direct inference endpoints for reset, approval/rejection, read/search/explain/file/code/business operations or error fallback. Gateway scope SHALL also be enforced deterministically before natural-language orchestrator tool dispatch and incidental ERPNext version calls, not only in frontend handlers. Local start-fresh SHALL make no deletion/ownership claim.

#### Scenario: Desk actions and failure paths
- **WHEN** chat, tool slash commands, stale approval cards, reset or gateway failure are exercised in Desk
