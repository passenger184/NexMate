...
Change `openspec/changes/m6-open-source-packaging/` (M6, 13 tasks, investigation-first). Offline investigation on this box (no `bench` binary on host PATH): repo root has no `pyproject.toml`/`setup.py`, so plain `bench get-app <repo-url>` is not expected to discover the app — supported path is `apps.json` with `directory: frappe_app` (added, untracked; JSON-validated). `frappe_app/pyproject.toml` name `erpnext_ai_copilot` + `tool.bench.frappe-dependencies`; `hooks.py` has all six `app_*` fields; `modules.txt` + 4 DocType dirs present. Self-containment proven offline: source `grep ^import/from config/tools/service/rag` in `frappe_app/` → 0; hard-coded path scan → 0 runtime hits (1 allowed docstring mention of `~/ERPNext-AI` in `proposals.py:87`); isolated `PYTHONPATH=frappe_app` subprocess imports `proposals`+`audit` OK; `tests/test_frappe_isolation.py` 4/4 OK; fallback `NEXMATE_DURABLE_DIR` → site private → app private (never `config.PROJECT_ROOT`). Decision: **Option A — Preserve monorepo** (recorded in `design.md` Decision Record 2026-09-19); Option B N/A (no move, no shim, no `service`/`rag`/`tools` move). Fresh `git clone file://repo /tmp/m6-fresh` OK: structure + 4 DocType dirs + `py_compile` OK + isolated import OK without `~/ERPNext-AI` on `PYTHONPATH` (`apps.json` untracked so absent from clone, as expected pre-commit). Offline suites 2026-09-19: `330` Python tests `OK (1 skipped)`, `67` Node PASS, `node --check` clean, `openspec validate --specs --strict` 10 pass, `git diff --check` clean. Guardrails held: no `NEXMATE_DURABLE_FALLBACK` in env, zero live business writes, no `~/projects/frappe_docker` access, no `docker restart/rebuild`, no inference calls, nothing staged.

**Live verification 2026-09-19 (user-authorized sync, `test-fresh-clone` only, frontend untouched):** the running test Bench (`frappe_docker_copilot_test-frontend-1`, `bench` 5.31.0, Frappe 16.31.0 / ERPNext 16.33.0 image, app 0.1.0) held a STALE pre-M5 copy with no pip metadata. Negative first: `bench get-app file:///tmp/m6-getapp-test` fails `FileNotFoundError: .../apps/m6-getapp-test/setup.py` (bench 5.31.0 expects root setup.py/pyproject) — plain get-app cannot discover the monorepo, confirming the `apps.json` `directory` path. Then: `rm` stale copy + `docker cp frappe_app` + `chown frappe:frappe` + drop `__pycache__` + `./env/bin/pip install -e apps/erpnext_ai_copilot` (0.1.0 OK) + `bench --site test-fresh-clone migrate` exit `0` (`Updating DocTypes for erpnext_ai_copilot: 100%`, after_migrate hooks). Verified: `frappe.get_all` → 4 DocTypes; `SHOW TABLES LIKE 'tabNexMate%'` → 4; counts `0 0 0 0`; `get_versions` → frappe 16.31.0 + app title/description (no TypeError); installed-context `import erpnext_ai_copilot.proposals` → path `apps/erpnext_ai_copilot/...`; `frappe.get_attr("erpnext_ai_copilot.proposals.TOOL_PROPOSAL_DOCTYPE")` → `NexMate Tool Proposal`. Synthetic `create_tool_proposal` via get_attr → fail-closed `ValidationError: NexMate gateway configuration error` (site key/url unconfigured — correct, no row, no inference call). (Note: bare `execute 'import ...'`/dotted-path expressions are invalid bench `execute` syntax — `execute` evals without the app imported — so valid dotted/get_attr forms were used.) Site has frappe + app only (no ERPNext), so no Customer check applies. Frontend NOT migrated/installed (read-only re-check: 4 DocTypes, 0 rows, apps list unchanged). Temp clones removed on both sides. All 13 M6 tasks complete; archived as `2026-09-19-m6-open-source-packaging`; M6 **CLOSED**.

## 2026-09-18 durable-tool-execution-audit — implementation and offline evidence

Change `openspec/changes/durable-tool-execution-audit/` (M5, 20/20 tasks): durable Frappe-owned `NexMate Tool Proposal` lifecycle (actor/site-bound, immutable payload/diff + SHA256, correlation, expiry 15 min, preconditions, advisory lock per target, idempotency key) with permission recheck at execution, serialized conflicting operations, uncertain-outcome reconciliation without blind replay; separate confined code/Git executor (root-contained, tracked-not-ignored, clean-tree, per-edit atomic commit) — inference never writes directly; correlated audit ledger (`NexMate Audit Entry`, actor/site/correlation/request/action linkage, redacted details, access-controlled, retention-aware, `audit_pending` repair); policy-filtered Debug view (minimized to actually accessed fields/results, never revealing denied sources, each view audited). Frappe envelope now carries durable proposal flows (`frappe-api-gateway` ADDED) and every tool path requires Frappe-authorized durable approval (`endpoint-access-control` ADDED). Contracts frozen in `tools/contracts.py` (bounded inputs/outputs, side-effect class, approval flag, typed errors, timeout, provenance). DocTypes `NexMate Tool Proposal` and `NexMate Audit Entry` plus `data/durable_proposals/` and `data/audit_ledger/` fallback for offline tests. Cutover: legacy RAM proposals become durably expired on first restart after cutover; prior JSONL `data/erpnext_writes.jsonl` remains readable, not authoritative; no production grants changed. Offline verification (dotenv disabled, mocked transports, `NEXMATE_DURABLE_FALLBACK=1`): `tests/test_durable` (durable lifecycle), `tests/test_audit_ledger` (correlated lifecycle, redaction, access, retention), `tests/test_debug_transparency` (authorized disclosure, minimization, audit), `tests/test_legacy_replacement` (direct inference refused, executor not bypassable, G3/G4 regressions), `tests/test_contracts` (explicit schemas) — **35 tests OK (1 skipped due to dirty tree)** for M5, plus 30 hardening + 4 isolation for **330 total (1 skipped)** after hardening (259 existing + 35 M5 + 30 hardening + 4 isolation); Node harness 67 checks pass (64 original + 3 new isolation), `node --check` clean, `openspec validate --specs --strict` 7 specs + 1 change pass, `git diff --check` clean, secret scans clean. Limits: live Bench/Docker verification, concurrent stress, and production-write approval remain future gates; `ARCHITECTURE.md` still describes pre-M5 executor/audit as target (separate HLD reconciliation, not edited here). Test data cleaned (fallback cleared). Deferred: U1 tenancy, U2 ingestion ownership, U5 permission parity beyond tiers, U8 versioning/packaging, U9 budgets, MCP/Workbench, SQL generation — all remain deferred per design.
